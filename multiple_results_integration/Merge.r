...
############################# merge function ###########################
#' @export Merge
Merge <- function(predResMat, ratio, autoOptimize = F ){
  ...
  if(autoOptimize == T){
    ...
    evalresult <- Evaluation(realScores = predResMat[,1],predScores = predResMat[,2:ncol(predResMat)],evalMethod = "pearson",topAlpha = 1:90)
