stacking <- function(predMat,evalMethods,by = 0.1,evaluation = T,topAlpha = 15, ...){
  ...
  radioEnsem <- Merge(predResMat = subEnsemble,ratio = c(0,1),autoOptimize = F)
  
  cycle <- seq(0,1,by = by)
  cycleLength <- length(cycle)
  
  for(ii in cycle[-1]){
    ensembleRes <- Merge(predResMat = subEnsemble,ratio = c(ii,1-ii),autoOptimize = F)
    radioEnsem <- cbind(radioEnsem,as.matrix(ensembleRes[,4]))
  }
  
  
  evalGYSSRatio <- Evaluation(realScores = radioEnsem[,1], predScores = radioEnsem[,4:(cycleLength + 3)], 
  ...
  for(i in 4:colNum){
    ...
    radioEnsem <- Merge(predResMat = subEnsemble,ratio = c(0*weight,1),autoOptimize = F)
    for(ii in cycle[-1]){
      ensembleRes <- Merge(predResMat = subEnsemble,ratio = c(ii*weight,1-ii),autoOptimize = F)
      radioEnsem <- cbind(radioEnsem,as.matrix(ensembleRes[,i+1]))
    }
    evalGYSSRatio <- Evaluation(realScores = radioEnsem[,1], predScores = radioEnsem[,(i+1):(i+cycleLength)], 
                                evalMethod = evalMethods,topAlpha = topAlpha, ...)
    if(evalMethods == "MSE"){
      maxEnsembleIdex <- order(evalGYSSRatio[[A]])[1]  
    }else{
  ...
  if(evaluation){
    evalRes <- Evaluation(realScores = finalMat[,1], predScores = finalMat[,2:(colNum+1)], 
                          evalMethod = evalMethods,topAlpha = topAlpha, ... )
...
############################# Ensemble function ###########################
#' @export Ensemble
Ensemble <- function(predMat, nrandom = 10, evalMethods, by = 0.1, evaluation = T, topAlpha = 15, ...){
  rowNum <- nrow(predMat)
  colNum <- ncol(predMat)
  ...
  for(i in 1:nrandom){
    ...
    res <- stacking(predMat = newRes, evalMethods = evalMethods,by = by,evaluation = evaluation,topAlpha = topAlpha)
...
#   pearsonMat <- pearsonMat[,-1]
  ...
  finalMat <- Merge(predResMat = predMat,ratio = weightMat[,index],autoOptimize = F)
  colnames(finalMat)[colNum + 1] <- "Ensemble"
